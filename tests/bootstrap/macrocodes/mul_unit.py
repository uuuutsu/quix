from quix.bootstrap.dtypes import Unit
from quix.bootstrap.dtypes.const import UCell
from quix.bootstrap.macrocodes import mul_unit
from quix.bootstrap.program import to_program
from quix.core.opcodes.opcodes import add

from .utils import run


def test_mul_unit_int() -> None:
    u1, u2 = Unit("u1"), Unit("u2")
    program = to_program(
        add(u1, 10),
        mul_unit(u1, UCell.from_value(20), u2),
    )

    mem = run(program)

    assert mem[u1] == 10
    assert mem[u2] == 200
    assert sum(mem.values()) == 210


def test_mul_unit_unit() -> None:
    u1, u2, u3 = Unit("u1"), Unit("u2"), Unit("u3")
    program = to_program(
        add(u1, 10),
        add(u2, 30),
        add(u3, 5),
        mul_unit(u1, u2, u3),
    )

    mem = run(program)

    assert mem[u1] == 10
    assert mem[u2] == 30
    assert mem[u3] == 44
    assert sum(mem.values()) == 84


def test_mul_unit_same_unit() -> None:
    u1, u2 = Unit("u1"), Unit("u2")
    program = to_program(
        add(u1, 10),
        add(u2, 5),
        mul_unit(u1, u1, u2),
    )

    mem = run(program)

    assert mem[u1] == 10
    assert mem[u2] == 100
    assert sum(mem.values()) == 110


def test_mul_unit_same_unit_each_arg() -> None:
    u1 = Unit("u1")
    program = to_program(
        add(u1, 10),
        mul_unit(u1, u1, u1),
    )

    mem = run(program)

    assert mem[u1] == 100
    assert sum(mem.values()) == 100
