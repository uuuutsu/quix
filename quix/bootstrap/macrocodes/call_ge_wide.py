...
def call_ge_wide(left: Wide, right: Wide, if_: CoreOpcode, else_: CoreOpcode) -> ToConvert:
    ge_flag = Unit(f"{left.name}_ge_{right.name}_flag")

    yield add(ge_flag, 1)
    yield _recursive_call_ge_wide_unit_flag(left.units, right.units, ge_flag)
    yield call_z_unit(ge_flag, else_, macrocode(add(ge_flag, -1), if_))
    return free(ge_flag)


@macrocode
def _recursive_call_ge_wide_unit_flag(left: tuple[Unit, ...], right: tuple[Unit, ...], ge_flag: Unit) -> ToConvert:
    ...
    return call_ge_unit(
        ...
        call_eq_unit(
            left[-1],
            right[-1],
            _recursive_call_ge_wide_unit_flag(left[:-1], right[:-1], ge_flag),
            NOP,
        ),
        add(ge_flag, -1),
    )
